...
1. `npm run clean` — clears stale output from previous builds
...
Runs `npm run build` then executes `node scripts/package.cjs`. The packaging script uses [JSZip](https://stuk.github.io/jszip/) to recursively bundle the entire `dist/` directory into a compressed zip archive. The output filename is derived from the `version` field in `package.json`:
